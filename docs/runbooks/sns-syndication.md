# Runbook: SNS 자동 송출 (Sanity → Bluesky / X / Instagram / LINE)
...
> 어댑터: `src/lib/syndicate/{bluesky,x,instagram,line}.ts`
...
- **현재 상태 (E-1)**: Bluesky 는 풀 구현, X / Instagram / LINE 은 stub.
  키만 채워두면 stub 은 `skipped` 로 응답해 다른 채널 송출은 정상.
...
### 3-1. Bluesky (현재 유일하게 동작하는 어댑터)

1. Bluesky 클라이언트 → **Settings** → **App Passwords** → **Add App Password**
2. 이름 `sokosoko-syndication` → 생성된 패스워드 복사 (한 번만 표시됨)
3. Vercel env vars 추가:

| Key                    | Value                                            |
| ---------------------- | ------------------------------------------------ |
| `BLUESKY_IDENTIFIER`   | 핸들 (`sokosoko.bsky.social`) 또는 이메일        |
| `BLUESKY_APP_PASSWORD` | 위에서 생성한 App Password (계정 본 비밀번호 ❌) |

> App Password 는 본 비밀번호와 별도로 회전 가능. 노출되면 Settings 에서
> 즉시 revoke.
...
> 어댑터는 현재 stub. 키를 채우면 `configured: true` 로 보고만 하고 실제
> 송출은 안 됨 (`skipped: x adapter not yet implemented`).

### 3-3. Instagram Graph API — stub
...
> ⚠️ Trigger 를 Update 로 잡으면 글 한 번 고칠 때마다 Bluesky 에 같은 글을
...
2. Sanity Manage → Webhooks → **sokosoko syndication** → **Logs**:
   ...
   - `results[]` 안에서 어댑터별 상태 확인:
     - Bluesky 키가 있으면 → `{ adapter: 'bluesky', configured: true, result: { status: 'ok', url: 'https://bsky.app/...' } }`
     - 키가 없으면 → `{ status: 'skipped', reason: 'not configured' }`
3. Bluesky 클라이언트에서 새 post 가 실제로 떴는지 확인.
...
| `results[].bluesky.status === 'error'` + `Invalid identifier or password` | Bluesky 자격 증명 오류                             | App Password 재발급 후 Vercel env var 갱신 |
...
> Bluesky App Password 는 위와 별도로, Bluesky Settings 에서 revoke + 재발급
> 후 Vercel env var (`BLUESKY_APP_PASSWORD`) 만 갱신하면 됨.
