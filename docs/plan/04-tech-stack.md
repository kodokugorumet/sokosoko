...
| CMS                | **Sanity v5** (Free tier)                              | Studio 는 임베디드 (`/studio`)                                                                   |
| Hosting            | **Vercel** (Hobby → Phase 2 시 Pro $20/월)             | Next.js 16 1st party, `@vercel/og`·`next/image` 무료 ([ADR-0003](../adr/0003-hosting-vercel.md)) |
| Edge middleware    | **`src/proxy.ts`** (Next 16 표준, Node.js 런타임)      | Sanity client / next-intl 모두 자유롭게 사용                                                     |
...
- **Cloudflare Pages + `next-on-pages`**: 어댑터 deprecated → CF Workers + OpenNext 시도 ([ADR-0002](../adr/0002-hosting-cloudflare-workers.md), superseded)
- **Cloudflare Workers + OpenNext**: Worker 크기 한계, Edge runtime 제약, OpenNext 베타 → Vercel 로 전환 ([ADR-0003](../adr/0003-hosting-vercel.md))
